...
// Compresses conversation history for context window management.
// Extracts key decisions, facts, topics, action items, and questions.
...
function extractActionItems(turns: ConversationTurn[]): string[] {
...
export function compressConversation(turns: ConversationTurn[], compressCount = 20): CompressResult {
  ...
  const decisions = toCompress.filter(t => decisionPatterns.test(t.text)).map(t => t.text.slice(0, 120));
  ...
  const facts = toCompress.filter(t => factPatterns.test(t.text)).map(t => t.text.slice(0, 120));
  ...
  const questions = toCompress.filter(t => questionPattern.test(t.text)).map(t => t.text.slice(0, 150));

  // Detect tone
  const toneWords: Record<string, RegExp> = {
...
export function formatSummaryAsBlock(summary: ConversationSummary): string {
