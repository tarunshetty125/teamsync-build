...
function buildSystemDesignInterviewOutputContract(): string {
    return [
        ...
        'Use real component names and real technologies (Kong, Envoy, Redis, Kafka, PostgreSQL, Cassandra, S3, CloudFront, OpenTelemetry) — no placeholders.',
        'The architecture_json must be valid JSON only inside the fence. No comments, no trailing commas.',
        'HARD QUALITY FLOOR: simple systems require 12+ nodes, medium production systems require 20+ nodes, FAANG-scale systems require 35-60+ nodes.',
        'BANNED: generic Frontend → Backend → Database diagrams. Low-quality diagrams below 12 nodes are invalid and will be rejected.',
        'Node schema: id, label, kind are required; technology, purpose, layer, latency, failureMode are optional for backward compatibility but expected for new output.',
        'Edge schema: source and target are required; label, protocol, latency are optional but expected for new output.',
        'Include layers: client, edge/gateway, core_services, async, data, cache, storage, search, observability, security.',
        'Use this production-grade shape:',
        '```architecture_json',
        '{"diagram":{"type":"architecture","direction":"TB","nodes":[{"id":"mobile","label":"Mobile Client","kind":"client","technology":"React Native","purpose":"User-facing app for reads/writes","layer":"client","latency":"~80ms","failureMode":"Offline queue"},{"id":"web","label":"Web Client","kind":"client","technology":"React SPA","purpose":"Browser interface","layer":"client"},{"id":"cdn","label":"CDN","kind":"storage","technology":"CloudFront","purpose":"Edge cache for static/media assets","layer":"edge"},{"id":"lb","label":"Load Balancer","kind":"gateway","technology":"AWS ALB","purpose":"TLS termination and health checks","layer":"edge","latency":"~5ms"},{"id":"gateway","label":"API Gateway","kind":"gateway","technology":"Kong + Envoy","purpose":"Auth, routing, rate limits","layer":"gateway","latency":"~10ms"},{"id":"auth","label":"Auth Service","kind":"service","technology":"Go + OAuth2","purpose":"Token validation and identity","layer":"security"},{"id":"core","label":"Core Service","kind":"service","technology":"Java + gRPC","purpose":"Main business workflow orchestration","layer":"core_services"},{"id":"worker","label":"Async Workers","kind":"service","technology":"Python Celery","purpose":"Background jobs and retries","layer":"async"},{"id":"redis","label":"Hot Cache","kind":"cache","technology":"Redis Cluster","purpose":"Sessions, hot reads, rate counters","layer":"cache","latency":"~2ms"},{"id":"kafka","label":"Event Bus","kind":"queue","technology":"Apache Kafka","purpose":"Durable async fanout","layer":"async"},{"id":"primary-db","label":"Primary Database","kind":"database","technology":"PostgreSQL","purpose":"Transactional source of truth","layer":"data"},{"id":"metrics","label":"Observability","kind":"external","technology":"OpenTelemetry + Prometheus","purpose":"Tracing, metrics, alerting","layer":"observability"}],"edges":[{"source":"mobile","target":"lb","label":"API requests","protocol":"HTTPS","latency":"~80ms"},{"source":"web","target":"cdn","label":"assets","protocol":"HTTPS"},{"source":"web","target":"lb","label":"API requests","protocol":"HTTPS"},{"source":"lb","target":"gateway","label":"routes","protocol":"HTTP/2"},{"source":"gateway","target":"auth","label":"verify token","protocol":"gRPC"},{"source":"gateway","target":"core","label":"business calls","protocol":"gRPC"},{"source":"core","target":"redis","label":"cache read/write","protocol":"Redis"},{"source":"core","target":"primary-db","label":"persist","protocol":"SQL"},{"source":"core","target":"kafka","label":"publish events","protocol":"Kafka"},{"source":"kafka","target":"worker","label":"consume jobs","protocol":"Kafka"},{"source":"core","target":"metrics","label":"telemetry","protocol":"OTLP"}]}}',
        '```',
