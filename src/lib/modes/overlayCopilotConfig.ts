...
import type { ActionContract, ContextTarget } from '../overlay/actionContextTypes';
...
export const ACTIONS: Record<OverlayQuickActionId, OverlayQuickActionDef> = {
  ...
  lecture_question: {
    ...
    additionalContext: 'Lecture mode: suggest sharp clarifying questions the learner should ask next to deepen understanding or resolve confusion.',
    profilePreference: 'force_off',
  },
  recruiting_strength: {
  ...
  job_improvement: {
    ...
    intent: 'what_to_answer',
    contextTarget: 'active_context',
    source: 'Improve Answer',
    analyticsKey: 'job_improvement',
    additionalContext: 'Improve action: use a critique-first structure. Start with **Weaknesses:** and identify 1-2 weak spots or missing details. Then use **Improvements:** to name the strongest fixes or follow-up points. End with **Improved Answer:** and provide the polished answer the user can say next. For coding contexts, keep the improved answer aligned with the active coding contract.',
  },
  system_tradeoffs: {
    id: 'system_tradeoffs',
    label: 'Tradeoffs',
...
const CONFIGS: Record<OverlayCopilotModeId, OverlayModeConfig> = {
  ...
  behavioral: {
    actionIds: ['job_star', 'job_improvement', 'job_confidence', 'clarify', 'brainstorm', 'follow_up_questions'],
    recommendedRules: [
      { actionId: 'answer_now', patterns: [/tell me about yourself/i, /introduce yourself/i] },
      { actionId: 'job_star', patterns: [/\btell me about a time\b/i, /\bexample\b/i, /\bexperience\b/i, /\bproject\b/i, /\bchallenge\b/i, /\bimpact\b/i] },
      ...
      { actionId: 'brainstorm', patterns: [/\bbrainstorm\b/i, /\boptions\b/i, /\bideas\b/i, /\bapproaches\b/i] },
      { actionId: 'clarify', patterns: [/\bclarify\b/i, /\bconstraint\b/i, /\bask\b/i] },
      { actionId: 'follow_up_questions', patterns: [/\bfollow.?up\b/i, /\bnext\b/i, /then what/i] },
    ],
    defaultRecommendedActionId: 'job_star',
  },
  coding: {
    actionIds: ['tech_hint', 'tech_optimal_solution', 'tech_complexity', 'tech_edge_case', 'follow_up_questions', 'job_improvement'],
    recommendedRules: [
      ...
      { actionId: 'follow_up_questions', patterns: [/\bfollow.?up\b/i, /\bnext question\b/i, /\bwhat.*ask\b/i] },
      { actionId: 'job_improvement', patterns: [/\bimprove\b/i, /\bbetter\b/i, /\bcleaner\b/i, /\brefactor\b/i, /\boptimi[sz]e my\b/i] },
  ...
  'looking-for-work': {
    actionIds: ['job_star', 'job_resume_alignment', 'job_confidence', 'job_improvement', 'clarify', 'brainstorm'],
    recommendedRules: [
      { actionId: 'answer_now', patterns: [/tell me about yourself/i, /introduce yourself/i] },
