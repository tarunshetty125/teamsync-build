...
const REGEX_SYSTEM_DIRECT = /\b(system design|systems design|high[\s-]?level design|\bhld\b|low[\s-]?level design|\blld\b|design (?:a|an|the)\s+(?:system|backend|architecture|platform|service|app|api|database|cache|queue|notification|feed|timeline|search|payments?|booking|ride(?:-|\s)?sharing(?: platform)?|rideshare|url shortener|tiny ?url|pastebin|paste ?bin|rate limiter|chat(?: application| app| system)?|file storage(?: system)?|parking lot(?: system)?|online code editor|code editor|library management(?: system)?|e[- ]?commerce(?: platform)?|messaging|social network|streaming|video|marketplace|storage|distributed system|distributed cache|distributed queue|logging platform|metrics platform|feature flag(?: system)?|recommendation (?:engine|system)|api gateway|search engine)|(?:design|build|scale|architect)\s+(?:twitter|instagram|uber|netflix|youtube|whatsapp|slack|discord|flipkart|swiggy|zomato|amazon|paytm|upi|google drive|dropbox|stripe|airbnb|linkedin|linkedin feed|kafka|redis|tiny ?url|pastebin)(?:\s+(?:backend|system|architecture|platform|service|app|feed))?|(?:design|build|architect)\s+(?:a |an |the )?(?:hld|lld|high[\s-]?level design|low[\s-]?level design)\s+(?:for|of)\s+(?:twitter|instagram|uber|netflix|youtube|whatsapp|slack|discord|flipkart|swiggy|zomato|amazon|paytm|upi|google drive|dropbox|stripe|airbnb|linkedin|kafka|redis|tiny ?url|pastebin|system|backend|platform|service|app)|redesign (?:the )?(?:backend|system|architecture|platform|service|app)|architecture of (?:the )?(?:system|backend|platform|service|app|database)|build(?:ing)? (?:a|an|the)\s+(?:platform|system|backend|service|app|api|database|cache|queue|notification|feed|timeline|search|payments?|booking|ride(?:-|\s)?sharing(?: platform)?|rideshare|url shortener|tiny ?url|pastebin|rate limiter|chat|messaging|social network|file storage|parking lot|code editor|library management|e[- ]?commerce|distributed cache|distributed queue|logging platform|metrics platform|feature flag|recommendation engine|api gateway|search engine))\b/;
const REGEX_SYSTEM_REAL_WORLD_DIRECT = /\b(?:design|build|architect|scale|create)\s+(?:a |an |the )?(?:(?:hld|lld|llf|high[\s-]?level design|low[\s-]?level design)\s+(?:for|of)\s+)?(?:flipkart|shopify|amazon|ecommerce|e-commerce|marketplace|cart|checkout|payment gateway|payments?|wallet|upi|paytm|stripe|whatsapp|chat|messaging|real[\s-]?time chat|live chat|notification(?: system| service)?|news feed|feed system|timeline|instagram|twitter|x app|youtube|netflix|video streaming|streaming platform|uber|lyft|ride booking|ride sharing|rideshare|food delivery|swiggy|zomato|doordash|booking|ticket booking|ticketmaster|airbnb|hotel booking|search engine|recommendation(?: system| engine)?|url shortener|tiny ?url|pastebin|paste ?bin|rate limiter|web crawler|file storage|dropbox|google drive|google docs|collaborative editor|slack|discord|parking lot|online code editor|code editor|library management|distributed cache|distributed queue|kafka|redis|logging platform|metrics platform|feature flag|api gateway|linkedin|linkedin feed)(?:\s+(?:backend|system|architecture|platform|service|app|hld|lld))?\b/;
...
const REGEX_SYSTEM_FRAMING = /\b(suppose|imagine|let s say|lets say|what if|consider|assume|scenario|in production|real[-\s]?world|in the real world|if suddenly|suddenly|at scale|in practice|what would happen|what happens|how would(?: you| we| this| the system)?|walk me through (?:the )?(?:architecture|system|backend|design|high[- ]?level design)|talk through (?:the )?(?:architecture|system|backend|design)|how would you design|how would you handle|how would you scale|how would you handle \d+ (?:million|billion|thousand) users?|how would you partition (?:the )?data|how would you handle fault tolerance|what database would you choose|how would you design the architecture|design a scalable)\b/;
