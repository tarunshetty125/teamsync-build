...
const UNIVERSAL_TRANSCRIPT_SIGNALS: Array<{
...
}> = [
  {
    patterns: [
      ...
      /(?:design|build|scale|architect)\s+(?:uber|instagram|instgram|netflix|netflx|youtube|whatsapp|watsapp|twitter|discord|slack|flipkart|flipkrat|flipcart|swiggy|zomato|amazon|shopify|paytm|upi|google drive|dropbox|stripe|airbnb|linkedin|kafka|redis|tiny ?url|pastebin|e-?commerce|marketplace|payment gateway|chat|messaging|notification|news feed|url shortener|rate limiter|search engine|recommendation|file storage|parking lot|code editor|library management|distributed cache|distributed queue|logging platform|metrics platform|feature flag|api gateway)/i,
      /(?:design|build|architect)\s+(?:a |an |the )?(?:hld|lld|llf|high[\s-]?level design|low[\s-]?level design)\s+(?:for|of)\s+(?:uber|instagram|instgram|netflix|netflx|youtube|whatsapp|watsapp|twitter|discord|slack|flipkart|flipkrat|flipcart|swiggy|zomato|amazon|shopify|paytm|upi|google drive|dropbox|stripe|airbnb|linkedin|kafka|redis|tiny ?url|pastebin|e-?commerce|marketplace|system|backend|platform|service|app)/i,
      /how would (?:you|we|this|the system).*(?:scale|design|handle|backend|service|database|cache|redis|shard|traffic|latency|throughput|recover|fail|retry|partition|architect)/i,
      /(?:redis|cache|database|backend|service|api|queue|kafka).*(?:fail|recover|retry|scale|traffic|shard)/i,
      /(?:million|millions|billion|100m|10m|qps|rps|traffic spike|high traffic|throughput|latency).*(?:backend|service|api|database|cache|queue|handle|scale|users?)/i,
      /(?:shard|sharding|caching|load balanc|fault toleran|single point of failure|avoid downtime|high availability)/i,
      /how would you handle \d+\s*(?:million|billion|thousand|k|m|b)\s*users?/i,
      /what database would you (?:choose|use|pick|recommend)/i,
      /how would you (?:partition|shard) (?:the )?data/i,
      /how would you (?:handle|ensure) (?:fault tolerance|high availability|reliability|consistency)/i,
      /how would you design the architecture/i,
      /design a scalable/i,
      /walk me through (?:the )?(?:high[- ]?level )?(?:architecture|design|system)/i,
    ],
