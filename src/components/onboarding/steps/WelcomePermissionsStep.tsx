...
export function WelcomePermissionsStep({ isAdvancing, onLaunch }: WelcomePermissionsStepProps) {
  ...
  // Show permissions modal after 1.5s — always show as visual element
  useEffect(() => {
  ...
  const handlePermissionAction = useCallback(
    (p: PermissionKind) => {
      const s = status?.[p];
      if (s === 'granted' || s === 'denied' || s === 'restart_required') void openSettings(p);
  ...
  const permissionsVisible = showPermissions && !dismissed;
  ...
  return (
    ...
    >
      ...
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/*  PERMISSIONS MODAL — Natively macOS dialog, pops after 1.5s      */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
      <AnimatePresence>
        {permissionsVisible && (
          <>
            ...
            >
              <div
                ...
                style={{
                  ...
                  background: '#ffffff',
                  borderRadius: '22px',
                  boxShadow: '0 32px 80px rgba(0,0,0,0.16), 0 8px 24px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.04)',
                  overflow: 'hidden',
                  display: 'grid',
                  gridTemplateColumns: '1fr 1fr',
                }}
              >
                ...
                <div className="p-8 pb-7" style={{ borderRight: '1px solid rgba(0,0,0,0.06)' }}>
                  {/* Eyebrow */}
                  ...
                  <h2
                    className="mt-4"
                    style={{ fontSize: '26px', fontWeight: 700, lineHeight: 1.14, color: '#1a1a2e', letterSpacing: '-0.02em' }}
                  >
                    ...
                    {PERMISSION_ROWS.map((perm, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <Icon
                              style={{ width: 18, height: 18, color: info.isGranted ? '#34c759' : '#6ec985' }}
                              strokeWidth={1.8}
                            />
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div style={{ fontSize: '14px', fontWeight: 600, color: '#1a1a2e' }}>{perm.label}</div>
                            <div
                              className="mt-0.5"
                              style={{
                ...
                {/* ── RIGHT COLUMN: 3D Wireframe + macOS dialog ── */}
                <div
                  className="flex flex-col items-center justify-center p-8 relative overflow-hidden"
                  style={{ background: '#08080f' }}
                >
                  {/* 3D Wireframe background */}
                  <div className="absolute inset-0 pointer-events-none wireframe-scene">
                    {/* Primary grid plane */}
                    <div className="wireframe-plane" />
                    {/* Secondary grid plane (parallax depth) */}
                    <div className="wireframe-plane wireframe-plane--layer2" />
                    {/* Intersection dots */}
                    <div className="wireframe-intersections" />
                  </div>
                  {/* Depth fade overlay */}
                  <div className="wireframe-depth-fade" />
                  {/* Horizon glow */}
                  <div className="wireframe-horizon-glow" />

                  {(() => {
                    const activePerm = PERMISSION_ROWS.find(p => {
                      ...
                      <>
                        ...
                        <motion.div
                          ...
                          style={{
                            ...
                            boxShadow: '0 8px 32px rgba(0,0,0,0.4), 0 0 1px rgba(255,255,255,0.08)',
                            display: 'flex',
                        ...
                        >
                          <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                              <div style={{
                                ...
                                color: 'rgba(255,255,255,0.88)', lineHeight: 1.3, letterSpacing: '-0.01em',
                              }}>
                                {dialogText}
                              </div>
                              <div style={{
                                fontSize: '9.5px', color: 'rgba(255,255,255,0.38)', lineHeight: 1.25,
                              }}>
                                {dialogSub}
                              </div>
                            </div>
                          ...
                          <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end', marginTop: '2px' }}>
                            <motion.button
                              type="button"
                              animate={!isGranted ? { scale: [1, 1.04, 1] } : {}}
                              transition={{ repeat: Infinity, duration: 2.2, ease: 'easeInOut', repeatDelay: 0.5 }}
                              onClick={() => void openSettings(activePerm.key)}
                              style={{
                                padding: '5px 10px', borderRadius: '5px',
                                background: isGranted ? 'rgba(52,199,89,0.18)' : 'rgba(255,255,255,0.06)',
                                border: `1px solid ${isGranted ? 'rgba(52,199,89,0.28)' : 'rgba(255,255,255,0.10)'}`,
                                fontSize: '9.5px', fontWeight: 600,
                                color: isGranted ? '#34c759' : 'rgba(255,255,255,0.80)',
                                letterSpacing: '-0.01em',
                                cursor: 'pointer',
                              }}
                            >
                              {isGranted ? '✓ Granted' : needsSettings ? 'Open Settings' : 'Allow'}
                            </motion.button>
                            {!isGranted && (
                              <div style={{
                                padding: '5px 10px', borderRadius: '5px',
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', zIndex: 2 }}>
                          <div style={{ width: '4px', height: '4px', borderRadius: '50%', background: 'rgba(255,255,255,0.18)', boxShadow: '0 0 4px rgba(120,160,255,0.2)' }} />
                          <div style={{ width: '1px', height: '14px', background: 'rgba(255,255,255,0.10)' }} />
                          <div style={{ width: '4px', height: '4px', borderRadius: '50%', background: 'rgba(255,255,255,0.18)', boxShadow: '0 0 4px rgba(120,160,255,0.2)' }} />
                        </div>

                        {/* System Settings toggle row */}
                        <motion.div
                          initial={{ opacity: 0, y: 10 }}
                          animate={{ opacity: 1, y: 0 }}
                          ...
                          style={{
                            ...
                            borderRadius: '10px',
                            padding: '8px 12px',
                            border: `1px solid ${isGranted ? 'rgba(52,199,89,0.22)' : 'rgba(255,255,255,0.08)'}`,
                          ...
                          onClick={() => handlePermissionAction(activePerm.key)}
                        >
                          <img
                            src={appIconDark}
                            alt="Quietly"
                          ...
                          />
                          <span style={{ fontSize: '11.5px', fontWeight: 550, color: 'rgba(255,255,255,0.85)', flex: 1, letterSpacing: '-0.01em' }}>
                            Quietly
                          </span>
                          {/* Toggle */}
                          <motion.div
                            animate={{
                              ...
                              boxShadow: isGranted
                                ? '0 0 8px rgba(52,211,153,0.3), 0 2px 4px rgba(0,0,0,0.2)'
