...
// ============================================
// Resume + JD Dashboard Extraction Types
// ============================================
...
export interface DashboardFeaturedProject {
    ...
    description: string;
    technologies: string[];
    extraCount: number; // computed post-processing, not from LLM
    // Optional fields added by post-processing/ranking
    confidence?: number; // 0-100 confidence score
    badges?: string[];
    impact?: { users?: number; pct?: number; summary?: string; impactScore?: number } | null;
    url?: string;
}

export interface DashboardAcademicBackground {
    degree: string;
    institution: string;
...
export interface DashboardExperience {
    ...
    endDate: string | null;
    isPresent: boolean;
    duration: string; // computed from startDate/endDate, not from LLM
    description: string;
}

export interface DashboardJobRequirements {
    role: string | null;
    requiredSkills: string[];
    preferredSkills: string[]; // deterministically inferred, not from LLM
    responsibilities: string[];
}

export interface DashboardMatchInsights {
    ...
    // Human-friendly explainability lines, e.g. ['+ React matched', '- Angular missing']
    explain?: string[];
    // Suggested skill-up paths with estimated match score improvement
    suggestions?: Array<{ skills: string[]; estimatedIncrease: number; text: string }>;
}
...
export interface DashboardExtractionResult {
    debug: DashboardDebugInfo;
    projects: DashboardFeaturedProject[];
    education: DashboardAcademicBackground[];
    skills: string[];
    experience: DashboardExperience[];
    jobRequirements: DashboardJobRequirements;
    matchInsights: DashboardMatchInsights;
    // Confidence per field (0-100) used by UI to render badges
    fieldConfidence?: { skills: number; projects: number; education: number; experience: number };
}
