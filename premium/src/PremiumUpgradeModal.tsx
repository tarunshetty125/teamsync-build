...
export const PremiumUpgradeModal: React.FC<PremiumUpgradeModalProps> = ({ isOpen, onClose, onActivated, onDeactivated, isPremium }) => {
    ...
    const handleActivate = async () => {
        ...
        try {
            ...
            console.log('[PremiumUpgradeModal] License activation result:', result);
            if (result?.success) {
                try {
                    localStorage.setItem('natively_premium_active', 'true');
                } catch {
                    // Ignore storage errors; premium state is still held by main process.
                }
                showToast('Pro unlocked', 'Your license has been activated.', 'success');
                setStatus('success');
                setTimeout(() => {
                    onActivated();
                    onClose();
                ...
                setStatus('error');
                const message = result?.error || 'Activation failed. Please try again.';
                setErrorMessage(message);
                showToast('Activation failed', message, 'error');
            }
        } catch (e: any) {
            setStatus('error');
    ...
    const handleDeactivate = async () => {
        try {
            ...
            console.log('[PremiumUpgradeModal] License deactivated');
            try {
                localStorage.setItem('natively_premium_active', 'false');
            } catch {
                // Ignore storage errors; premium state is still held by main process.
            }
            showToast('License removed', 'Pro features disabled on this device.', 'neutral');
            onDeactivated?.();
            onClose();
        } catch (e: any) {
            const message = e.message || 'Deactivation failed.';
            setErrorMessage(message);
            showToast('Deactivation failed', message, 'error');
        }
    };
