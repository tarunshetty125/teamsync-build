...
interface MaxUltraUpgradeToasterProps {
    className?: string;
    isOpen: boolean;
    onDismiss: () => void;
    onUpgrade: () => void;
...
export const MaxUltraUpgradeToaster: React.FC<MaxUltraUpgradeToasterProps> = ({
    className,
    isOpen,
    onDismiss,
    onUpgrade,
    ...
    const handleDismiss = () => {
        localStorage.setItem('teamsync_max_ultra_toaster_dismissed', Date.now().toString());
        onDismiss();
    };
                ...
                <div className={`fixed inset-0 z-[9998] flex items-center justify-center ${isLight ? 'bg-black/30 backdrop-blur-[2px]' : 'bg-black/60 backdrop-blur-[2px]'}`}>
                    ...
                    >
                        <div className="flex w-full flex-col items-center px-[40px] pt-[36px] text-center">
                            <div className={`relative mb-[20px] flex h-[48px] w-[48px] items-center justify-center rounded-full border shadow-inner ${isLight ? 'border-[#F97316]/25 bg-[#F97316]/10' : 'border-white/10 bg-white/5'}`}>
                                <div className="absolute inset-0 rounded-full bg-[#F97316] opacity-20 blur-[24px]" />
                                <Crown size={24} className="relative z-10 text-[#F97316]" strokeWidth={1.5} />
                            </div>

                            <h3 className={`mb-[10px] text-[24px] font-[600] leading-[1.2] tracking-[-0.01em] antialiased ${isLight ? 'text-[#1C1C1E]' : 'text-[#F3F3F3]'}`}>
                                Upgrade to Max or Ultra.
                            </h3>
                            <p className={`max-w-[420px] text-[14px] font-medium leading-[1.6] antialiased ${isLight ? 'text-black/50' : 'text-white/60'}`}>
                                Unlock the highest-capacity TeamSync plans for heavier interview sessions, profile intelligence, and premium context features.
                            </p>
                        </div>

                        <div className="mt-7 mb-8 grid w-full grid-cols-3 gap-2 px-[32px]">
                            {['More usage', 'Profile depth', 'Priority models'].map((label) => (
                                <div
                                    key={label}
                        ...
                        <button
                            ...
                            className="group relative mb-[16px] h-[48px] w-[320px] overflow-hidden rounded-[16px] border border-white/5 shadow-[0_4px_16px_rgba(249,115,22,0.15)] transition-all duration-300 hover:scale-[1.01] hover:shadow-[0_8px_24px_rgba(249,115,22,0.25)] active:scale-[0.99]"
                        >
                            <div className="absolute inset-0 bg-gradient-to-b from-[#FB923C] to-[#EA580C]" />
